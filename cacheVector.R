## This is an example of how to cache computations to reduce run time.
...
makeVector <- function(x = numeric()) {
        m <- NULL
        set <- function(y) {
                x <<- y         ## Assigns the value of y to the x in the parent function
                m <<- NULL      ## Assigns m in parent function to NULL
        }
        get <- function() x
        setmean <- function(mean) m <<- mean    ## Assigns the variable m in the parent function
        getmean <- function() m
